...
# 2. Bot Setup & Intents with full coverage
intents = discord.Intents.all()

# Secure JSON Storage File for Security and Data
    ...
    try:
        ...
        if channel and isinstance(channel, discord.VoiceChannel):
            ...
            if channel.name != new_name:
                await channel.edit(
                    name=new_name, reason="Updating member count stats."
                )
    except Exception as e:
...
class TicketControlView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="persistent_ticket_close_btn",
        emoji="🔒",
    )
    async def close_ticket(
        ...
        try:
            ...
            closed_cat = (
                guild.get_channel(self.closed_category_id)
                if self.closed_category_id
                else None
            )

            await channel.edit(
                name=f"closed-{interaction.user.name}",
                category=closed_cat,
            ...
            await interaction.followup.send(
                "Ticket closed successfully! 🔒", ephemeral=True
            )

            open_view = TicketOpenView(self.closed_category_id)
            await channel.send(
                "This ticket has been closed. Choose an action below:", view=open_view
            )
        except Exception as e:
            await interaction.followup.send(
                f"Error closing ticket: {e}", ephemeral=True
            )
...
class TicketOpenView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="persistent_ticket_reopen_btn",
        emoji="🔓",
    )
    async def reopen_ticket(
        ...
        try:
            channel = interaction.channel
            await channel.edit(
                name=f"ticket-{interaction.user.name}", reason="Ticket reopened."
            )
            await interaction.followup.send(
            ...
            await interaction.followup.send(
                f"Error reopening ticket: {e}", ephemeral=True
            )

    @discord.ui.button(
        label="Delete",
        style=discord.ButtonStyle.grey,
        custom_id="persistent_ticket_delete_btn",
        emoji="🗑️",
    )
    async def delete_ticket(
        ...
        await interaction.response.send_message(
            "Deleting ticket channel in 3 seconds...", ephemeral=True
        )
...
class TicketSelect(discord.ui.Select):
    ...
    def __init__(self, reasons_list, open_category_id, closed_category_id):
            ...
            if label:
                options.append(
                    discord.SelectOption(
                        label=label, description=desc[:100], emoji="🎫"
                    )
                )
        ...
        if not options:
            options.append(
                discord.SelectOption(
                    label="Support", description="General Support", emoji="🎫"
                )
            )

        max_val = min(len(options), 25)
        super().__init__(
            placeholder="Select reasons to open a ticket...",
            min_values=1,
            max_values=max_val,
            options=options,
            custom_id="persistent_ticket_select_menu",
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            category = (
                guild.get_channel(self.open_category_id)
...
class TicketSelectView(discord.ui.View):
    ...
    def __init__(
        self, reasons_list, open_category_id=None, closed_category_id=None
    ):
        super().__init__(timeout=None)
        self.add_item(
            TicketSelect(reasons_list, open_category_id, closed_category_id)
        )
...
# --- SECURITY & PUNISHMENT EXECUTOR HELPER ---
...
async def apply_punishment(guild, member, action_type, channel_id=None):
    ...
    try:
        ...
        if log_channel_id:
            ...
            if log_chan:
                await log_chan.send(
                    f"🛡️ **Security Triggered:** Member {member.mention} was"
                    f" **{punishment}ned** for: `{action_type}`"
                )
    except Exception as e:
...
@bot.tree.command(
    name="setpunishment",
    description=(
        "Set punishment for security actions with optional specific channel"
    ),
)
...
@app_commands.choices(
    action_type=[
        app_commands.Choice(
            name="Delete Message in Channel", value="del_msg"
        ),
        ...
        app_commands.Choice(
            name="Giving Administrator Role", value="give_admin"
        ),
...
async def setpunishment(
    ...
    await interaction.response.send_message(
        f"✅ Punishment for **{action_type}** set to **{punishment}** in"
        f" **{chan_text}**",
        ephemeral=True,
    )
...
@bot.tree.command(
    ...
    description="Set the security logs channel for actions",
)
...
async def setlogssecurity(
    ...
    save_data(db)
    await interaction.response.send_message(
        f"✅ Security logs channel successfully set to: {channel.mention}",
        ephemeral=True,
    )
...
async def antinuke(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"🔒 Antinuke status is now: **{status}**", ephemeral=True
    )


@bot.tree.command(
    name="antiraid", description="Toggle the Anti-Raid security system"
)
@app_commands.checks.has_permissions(administrator=True)
async def antiraid(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"🛡️ Anti-Raid status is now: **{status}**", ephemeral=True
    )


@bot.tree.command(
    name="rudewordadd",
    description="Add a bad word to the filter with an auto-reply message",
)
@app_commands.describe(
    word="The word to filter",
    delete_message="Delete the message?",
    reply_message="Bot reply message (e.g. Shut up @user!)",
)
@app_commands.choices(
    delete_message=[
...
async def rudewordadd(
    ...
    save_data(db)
    await interaction.response.send_message(
        f"✅ Word `{word}` has been added to the bad words filter.", ephemeral=True
    )


@bot.tree.command(
    name="addblacklistserver",
    description="Blacklist a server ID from letting users join",
)
...
async def addblacklistserver(interaction: discord.Interaction, server_id: str):
    if server_id not in db["blacklist_servers"]:
        ...
        save_data(db)
        await interaction.response.send_message(
            f"✅ Server `{server_id}` added to blacklist.", ephemeral=True
        )
    else:
        await interaction.response.send_message(
            f"⚠️ Server is already blacklisted.", ephemeral=True
        )
...
async def joinvc(interaction: discord.Interaction, channel_id: str):
    try:
        ...
        if isinstance(channel, discord.VoiceChannel):
            ...
            await interaction.response.send_message(
                f"🔊 Successfully joined voice channel: `{channel.name}`",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
                "❌ Invalid or unavailable voice channel ID.", ephemeral=True
            )
    except Exception as e:
        await interaction.response.send_message(
...
@bot.tree.command(
    name="leavevc", description="Make the bot leave the voice channel"
)
async def leavevc(interaction: discord.Interaction):
    if interaction.guild.voice_client:
        ...
        await interaction.response.send_message(
            "⚠️ The bot is not connected to any voice channel.", ephemeral=True
        )


@bot.tree.command(
    name="voiceinfo", description="Show current voice connection info"
)
async def voiceinfo(interaction: discord.Interaction):
    if interaction.guild.voice_client and interaction.guild.voice_client.channel:
        ...
        await interaction.response.send_message(
            f"📊 Voice Info:\n- Channel:"
            f" {vc.channel.name}\n- Connected Members:"
            f" {len(vc.channel.members)}",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
            "⚠️ The bot is not connected to voice currently.", ephemeral=True
        )
...
async def warn(
    ...
    save_data(db)
    await interaction.response.send_message(
        f"⚠️ Member {member.mention} has been warned. Reason: {reason}",
        ephemeral=True,
    )
...
async def check_warns(interaction: discord.Interaction, member: discord.Member):
    ...
    if not warns:
        await interaction.response.send_message(
            f"✅ Member {member.mention} has no warnings.", ephemeral=True
        )
    else:
...
async def clear_warns(interaction: discord.Interaction, member: discord.Member):
    ...
    if guild_id in db["warns"] and user_id in db["warns"][guild_id]:
        ...
        await interaction.response.send_message(
            f"⚠️ Member has no warnings recorded.", ephemeral=True
        )
...
async def mute(
    ...
    await interaction.response.send_message(
        f"⚠️ Member {member.mention} has been muted for {duration}. Reason:"
        f" {reason}",
        ephemeral=True,
    )
...
async def kick(
    ...
    await interaction.response.send_message(
        f"👢 Kicked {member.mention}. Reason: {reason}", ephemeral=True
    )
...
async def ban(
    ...
    await interaction.response.send_message(
        f"🔨 Banned {member.mention}. Reason: {reason}", ephemeral=True
    )


@bot.tree.command(
    name="unban",
    description="Unban a member by name or type 'all' to unban everyone",
)
...
async def unban(interaction: discord.Interaction, member_name: str):
    ...
    if member_name.lower() == "all":
        if not ban_entries:
            await interaction.followup.send(
                "⚠️ There are no banned members in this server.", ephemeral=True
            )
            return
        ...
        for entry in ban_entries:
            try:
                ...
                pass
        await interaction.followup.send(
            f"✅ Successfully unbanned all banned members (Total: {count}).",
            ephemeral=True,
        )
        return
    ...
    for entry in ban_entries:
        if entry.user.name.lower() == member_name.lower():
            await interaction.guild.unban(entry.user)
            await interaction.followup.send(
                f"✅ Successfully unbanned `{member_name}`.", ephemeral=True
            )
            return

    await interaction.followup.send(
        f"❌ Member `{member_name}` not found in bans list.", ephemeral=True
    )
...
async def stats(interaction: discord.Interaction):
    guild = interaction.guild
    embed = discord.Embed(
        title=f"📊 Statistics for {guild.name}", color=discord.Color.blue()
    )
    embed.add_field(
...
@bot.tree.command(
    name="embed", description="Send a custom professional embed message"
)
@app_commands.checks.has_permissions(administrator=True)
async def embed(
    interaction: discord.Interaction,
...
# --- MANUAL SYNC COMMAND TO FIX COMMANDS DISAPPEARING ---
...
async def on_guild_role_create(role):
    ...
    try:
        async for entry in role.guild.audit_logs(
            limit=3, action=discord.AuditLogAction.role_create
        ):
            if entry.user and not entry.user.bot:
...
async def on_member_update(before, after):
    await asyncio.sleep(1.5)
    try:
        if not any(r.permissions.administrator for r in before.roles) and any(
            r.permissions.administrator for r in after.roles
        ):
        ...
        async for entry in guild.audit_logs(
            ...
            if entry.user and not entry.user.bot:
                await apply_punishment(
                    guild, entry.user, "del_msg", channel_id=payload.channel_id
                )
                return
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if TOKEN:
        ...
        print("❌ Error: TOKEN or DISCORD_TOKEN environment variable is not set!")
