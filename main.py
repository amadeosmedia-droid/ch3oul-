...
async def update_online_count_channel(guild: discord.Guild):
    saved_counter = DATA.get("online_count", {}).get(str(guild.id), 0)
    channel_id = int(saved_counter or 0)
    if not channel_id:
        return
    channel = guild.get_channel(channel_id)
    ...
    try:
        ...
        if channel.name != new_name:
            await channel.edit(name=new_name, reason="Updating online member count.")
    except discord.Forbidden:
        print(f"[ONLINE COUNT] Missing permission in {guild.name}")
    except Exception as exc:
        print(f"[ONLINE COUNT] {exc}")
...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        try:
            ...
            await update_online_count_channel(guild)
...
async def on_presence_update(before: discord.Member, after: discord.Member):
    try:
        await update_online_count_channel(after.guild)
    except Exception as exc:
        print(f"[ONLINE PRESENCE] {exc}")
...
async def connect_voice(channel: discord.VoiceChannel, *, self_deaf=True):
    """Connect/move to voice safely, preventing concurrent connect races."""
    guild = channel.guild
    me = guild.me
    if me is None:
    ...
    if not perms.connect:
        raise discord.Forbidden(discord.Object(id=0), "Missing Connect permission for the selected voice channel.")
    ...
    lock = VOICE_CONNECT_LOCKS.setdefault(guild.id, asyncio.Lock())
    async with lock:
        # Re-check after acquiring the lock: another command may have connected
        ...
        for attempt in range(1, 4):
            try:
                # A second check closes the small race window before channel.connect().
                existing = guild.voice_client
                ...
                print(f"[VOICE] Connecting to {channel.name} (attempt {attempt}/3, self_deaf={self_deaf})...")
                voice = await channel.connect(timeout=30.0, reconnect=True, self_deaf=self_deaf)
                print(f"[VOICE] Connected to {channel.name} in {guild.name} (opus_loaded={discord.opus.is_loaded()})")
                return voice
            except discord.ClientException as exc:
                # Discord can report "Already connected" if the gateway state
                # changed between our check and connect(). Reuse that client.
                current = guild.voice_client
                if current is not None and current.is_connected():
                    print(f"[VOICE] Connection already exists; reusing it for {guild.name}.")
                    if current.channel != channel:
                        await current.move_to(channel)
                    return current
                last_exc = exc
                print(f"[VOICE ERROR] {guild.name} attempt {attempt}: {voice_exception_details(exc)}")
                traceback.print_exc()
            except Exception as exc:
                last_exc = exc
                print(f"[VOICE ERROR] {guild.name} attempt {attempt}: {voice_exception_details(exc)}")
...
@bot.tree.command(name="status", description="Show the bot status and server count.")
async def status(interaction: discord.Interaction):
    ...
    embed.add_field(name="Uptime", value="Online and ready", inline=False)
...
async def lock(interaction: discord.Interaction):
    ...
    channel = interaction.channel
    if not isinstance(channel, discord.TextChannel):
        return await interaction.response.send_message("❌ This command only works in a text channel.", ephemeral=True)
    try:
...
@bot.tree.command(name="top", description="Show the top members by highest role position.")
async def top(interaction: discord.Interaction):
    guild = interaction.guild
    if guild is None:
        return await interaction.response.send_message("Use this command in a server.", ephemeral=True)
    members = sorted(
        [m for m in guild.members if not m.bot and m != guild.owner],
        key=lambda m: (m.top_role.position, m.joined_at.timestamp() if m.joined_at else 0),
        reverse=True,
    )[:10]
    lines = []
    if guild.owner:
        lines.append(f"👑 **1.** {guild.owner.mention} — Owner")
    for i, member in enumerate(members, start=2 if guild.owner else 1):
        lines.append(f"**{i}.** {member.mention} — {member.top_role.mention}")
    embed = discord.Embed(title="🏆 Top Members", description="\n".join(lines) or "No members found.", color=discord.Color.gold())
    await interaction.response.send_message(embed=embed)
...
@app_commands.describe(user="Member whose avatar you want to see.")
...
async def commands_list(interaction: discord.Interaction):
    names = sorted(c.name for c in bot.tree.get_commands() if c.name)
    text = "\n".join(f"`/{name}`" for name in names)
    embed = discord.Embed(title="📜 Bot Commands", description=text or "No commands available.", color=discord.Color.blurple())
    await interaction.response.send_message(embed=embed, ephemeral=True)


# -------------------- /CREATEVOICEONLINE --------------------

@bot.tree.command(name="createvoiceonline", description="Create a voice channel showing the number of online members.")
async def createvoiceonline(interaction: discord.Interaction):
    error = check_command(interaction, "manage_channels")
    if error:
        return await send_command_error(interaction, error)
    guild = interaction.guild
    try:
        existing_id = int(DATA.get("online_count", {}).get(str(guild.id), 0) or 0)
        channel = guild.get_channel(existing_id) if existing_id else None
        online = sum(1 for m in guild.members if not m.bot and m.status != discord.Status.offline)
        name = f"🟢・online:{online}"
        if not isinstance(channel, discord.VoiceChannel):
            channel = await guild.create_voice_channel(name, reason=f"Online counter created by {interaction.user}")
        elif channel.name != name:
            await channel.edit(name=name, reason="Updating online counter.")
        DATA.setdefault("online_count", {})[str(guild.id)] = channel.id
        save_data()
        await interaction.response.send_message(f"✅ Online counter created: {channel.mention}", ephemeral=True)
    except discord.Forbidden:
        await interaction.response.send_message("❌ I need Manage Channels permission.", ephemeral=True)
    except Exception as exc:
        print(f"[CREATE ONLINE] {exc}")
        await interaction.response.send_message("❌ Could not create the online counter.", ephemeral=True)
...
async def extract_audio(query: str):
    ...
    # YouTube is increasingly challenging datacenter IPs. We try several
    # yt-dlp clients first. If YouTube blocks the Render IP, fall back to
    # SoundCloud search for normal song-name searches so /play still works.
    cookie_file = None
    cookie_b64 = os.getenv("YOUTUBE_COOKIES_B64", "").strip()
    if cookie_b64:
        try:
            raw = base64.b64decode(cookie_b64).decode("utf-8")
            fd, cookie_file = tempfile.mkstemp(prefix="ytcookies_", suffix=".txt")
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                f.write(raw)
        except Exception as exc:
            print(f"[YOUTUBE COOKIES] Could not decode YOUTUBE_COOKIES_B64: {exc}")
            cookie_file = None
    ...
    try:
        try:
            ...
            print(f"[YOUTUBE] yt-dlp version: {probe.stdout.strip()}")
        except Exception as exc:
            print(f"[YOUTUBE] yt-dlp probe failed: {exc}")
        try:
            import subprocess
            diag = subprocess.run(
                ["python", "-m", "yt_dlp", "-v", "--simulate",
                 "--extractor-args",
                 "youtubepot-bgutilhttp:base_url=http://127.0.0.1:4416",
                 "https://www.youtube.com/watch?v=GVMKE0WQlLE"],
                capture_output=True, text=True, timeout=20,
            )
            diag_text = (diag.stdout or "") + (diag.stderr or "")
            pot_lines = [line for line in diag_text.splitlines() if "PO Token Providers" in line or "bgutil" in line.lower()]
            if pot_lines:
                print("[YOUTUBE] bgutil diagnostic:")
                for line in pot_lines[:8]:
                    print(line)
            else:
                print("[YOUTUBE] WARNING: yt-dlp did not report a bgutil provider in diagnostic output.")
        except Exception as exc:
            print(f"[YOUTUBE] bgutil diagnostic failed: {exc}")

        # YouTube clients currently documented by yt-dlp as useful when
        # PO-token enforcement varies. Keep each attempt isolated so one
        # blocked client does not prevent the next one from being tried.
        # With bgutil installed and its local provider running, the normal
        # yt-dlp YouTube extractor can obtain fresh PO tokens automatically.
        # Keep a few lightweight client fallbacks for videos with client-specific
        # restrictions.
        # Each tuple is (client, legacy_mode). We deliberately include both
        # HTTP and script PO-token providers. This makes the bot survive if the
        # local HTTP provider starts slowly or fails to bind on Render.
        client_attempts = [
            ("mweb", False),
            ("web_embedded", False),
            ("mweb", True),
        ...
        for player_client, legacy_mode in client_attempts:
            opts = {
            ...
            # Explicitly point the bgutil plugin at the local PO-token server.
            # The provider plugin normally defaults to 127.0.0.1:4416, but making
            # it explicit avoids Render/runtime environment ambiguity.
            # Use both official bgutil modes. HTTP is faster; script mode is a
            # local fallback and does not depend on the provider server being
            # ready. The server_home path is the one built during Render deploy.
            bgutil_server_home = os.path.abspath("bgutil-ytdlp-pot-provider/server")
            youtube_args = {
                "fetch_pot": "always",
                "formats": "missing_pot",
            }
            ...
            try:
                mode = "legacy" if legacy_mode else "normal"
                print(f"[YOUTUBE] Trying client: {player_client or 'default'} ({mode})")
                print(f"[YOUTUBE] bgutil server_home: {bgutil_server_home}")
                ...
                def run_extract():
                    with yt_dlp.YoutubeDL(opts) as ydl:
                        ...
                        if "entries" in info:
                            ...
                            if not entries:
                                raise RuntimeError("YouTube returned no playable result")
                            info = entries[0]
                        stream_url = info.get("url")
                        if not stream_url:
                            raise RuntimeError("YouTube returned no playable audio URL")
                        return {
                            "url": stream_url,
                ...
                result = await asyncio.to_thread(run_extract)
                print(
                    f"[YOUTUBE] Success with client: "
                    f"{player_client or 'default'}"
                )
                return result
            except Exception as exc:
                last_error = exc
                print(
                    f"[YOUTUBE] Client {player_client or 'default'} failed: "
                    f"{type(exc).__name__}: {exc}"
                )
                continue

        # If the user supplied a normal song/search phrase, use SoundCloud
        # as a second provider. This is intentionally not used for a direct
        # YouTube URL because the requested video identity would be lost.
        is_direct_url = query.startswith((
            "http://", "https://", "www.youtube.com/", "youtube.com/", "youtu.be/"
        ))
        if not is_direct_url:
            sc_query = query if query.lower().startswith("scsearch") else f"scsearch1:{query}"
            print(f"[MUSIC FALLBACK] YouTube blocked; trying SoundCloud: {query}")
            sc_opts = {
                "format": "bestaudio/best",
                "noplaylist": True,
                "quiet": True,
            ...
            try:
                def run_soundcloud():
                    with yt_dlp.YoutubeDL(sc_opts) as ydl:
                        ...
                        if "entries" in info:
                            entries = [e for e in info["entries"] if e]
                            if not entries:
                                raise RuntimeError("SoundCloud returned no playable result")
                            info = entries[0]
                        stream_url = info.get("url")
                        if not stream_url:
                            raise RuntimeError("SoundCloud returned no playable audio URL")
                        return {
                            "url": stream_url,
                            "title": info.get("title", "Unknown"),
                            "webpage_url": info.get("webpage_url", query),
                ...
                result = await asyncio.to_thread(run_soundcloud)
                print(f"[MUSIC FALLBACK] SoundCloud success: {result['title']}")
                return result
            except Exception as exc:
                last_error = exc
                print(
                    f"[MUSIC FALLBACK] SoundCloud failed: "
                    f"{type(exc).__name__}: {exc}"
                )
    ...
    raise last_error or RuntimeError("Could not extract playable audio")
