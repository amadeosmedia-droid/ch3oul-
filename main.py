...
async def extract_audio(query: str):
    ...
    # YouTube increasingly challenges datacenter IPs. Try a few clients that
    # currently work without account cookies/PO tokens before giving up.
    # Optional cookies can be supplied through YOUTUBE_COOKIES_B64.
    import base64
    import tempfile
    import os
    ...
    try:
        for player_client in client_attempts:
            ...
            try:
                ...
                print(f"[YOUTUBE] Success with client: {player_client or 'default'}")
                return result
            except Exception as exc:
                last_error = exc
                print(f"[YOUTUBE] Client {player_client or 'default'} failed: {type(exc).__name__}: {exc}")
                continue
    finally:
        if cookie_file:
            try:
    ...
    raise last_error or RuntimeError("Could not extract audio from YouTube")
